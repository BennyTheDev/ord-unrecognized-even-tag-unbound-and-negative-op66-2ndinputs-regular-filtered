...
impl Inscription {
  #[cfg(test)]
  pub(crate) fn new(content_type: Option<Vec<u8>>, body: Option<Vec<u8>>, is_op66 : bool) -> Self {
    Self { content_type, body, is_op66 }
  }

  // hack helper
  pub(crate) fn from_transaction_2nd(tx: &Transaction) -> Result<Inscription> {
  ...
  // hack helper
  pub(crate) fn from_transaction_1st(tx: &Transaction) -> Result<Inscription> {
  ...
  // quick hack to pull either from input 0 or 1
  pub(crate) fn from_transaction(tx: &Transaction) -> Result<Inscription> {

    let valid = match Inscription::from_transaction_1st(tx) {
      Ok(_inscription) => true,
      Err(InscriptionError::UnrecognizedEvenField(_inscription)) => false,
      _ => false,
    };
  ...
  fn parse_inscription(&mut self) -> Result<Option<Inscription>> {
    if self.advance()? == Instruction::Op(opcodes::all::OP_IF) {
      ...
      if cursed_tag.is_some() {
        return Ok(Some(Inscription { body, content_type, is_op66 : true }));
      }

      return Ok(Some(Inscription { body, content_type, is_op66 : false }));
    }
