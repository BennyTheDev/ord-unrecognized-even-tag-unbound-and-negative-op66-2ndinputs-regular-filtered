...
impl<'a, 'db, 'tx> InscriptionUpdater<'a, 'db, 'tx> {
  ...
  pub(super) fn index_transaction_inscriptions(
    ...
    // defining these as cursed as well later on below
    let first_input_inscription = Inscription::from_transaction_1st(tx).is_ok();
    let second_input_inscription = Inscription::from_transaction_2nd(tx).is_ok();
    ...
    // trying not to include positive ones as their numbers will be off anyway
    if inscriptions.iter().all(|flotsam| flotsam.offset != 0) &&
        inscription.is_some()
    {

      let res = inscription.map(|inscription|inscription.is_op66);

      let is_op66 = match res {
        Some(p) => p,
        None => false
      };

      if cursed || is_op66 || (!first_input_inscription && second_input_inscription)
      {
